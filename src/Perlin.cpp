...
double Perlin_Noise1(double arg)
{
    int bx0, bx1;
...
static void Perlin_Normalize2(float v[2])
{
...
static void Perlin_Normalize3(float v[3])
{
...
static void Perlin_Init(void)
{
    ...
    for (i = 0; i < B; i++) {
        ...
        Perlin_Normalize2(g2[i]);
        
        for (j = 0; j < 3; j++)
            g3[i][j] = (float)((rand() % (B + B)) - B) / B;
        Perlin_Normalize3(g3[i]);
    }
